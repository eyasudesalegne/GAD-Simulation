...
class HPAAxis:
    """
    Hypothalamic-Pituitary-Adrenal axis model simulating cortisol feedback.
    """
...
class Neuromodulator:
    """
    Models extracellular concentrations and receptor states of key neuromodulators.
    Includes dopamine, serotonin, norepinephrine, acetylcholine, GABA, and cortisol.
    """
    ...
    def __init__(self, dt, duration, compartments=None, diffusion_matrix=None, noise_std=0.005,
        ...
        # Initialize concentrations per compartment (0 to 1 scale)
        self.concentrations = {k: np.full(self.N, 0.2) for k in ['DA', '5-HT', 'NE', 'ACh', 'GABA', 'Cort']}

        self.hpa_axis = HPAAxis(dt)

        # Receptor states: Active (A), Bound (B), Internalized (I)
        self.receptors = {
            'DA': {
    ...
    def get_receptor_activation(self, nm, rec_type):
        """
        Returns fraction of bound receptors (activation) for neuromodulator nm and receptor type rec_type.
        """
    ...
    def co_modulation(self):
        """
        Dopamine-serotonin co-modulation logic.
        """
    ...
    def update_receptors(self):
        """
        Update receptor states according to kinetic parameters.
        """
    ...
    def step(self, external_inputs, stress_input):
        """
        Advance neuromodulator dynamics by one step.
        """
